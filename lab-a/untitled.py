...
class Domain:
    ...
    def everything(self):
        ...
        return everything
# maze = [[0 for i in range(Domain(data).count_columns())] for j in range (Domain(data).count_rows())]
...
class Node:
    ...
    def __init__(self, state, move=None, parent=None,cost=None):
            self._state = state
...
def heuristic(node,domain):
    heuristic = node.get_state()[0][0] - domain.prize_point()[0][0]
    heuristic2 = node.get_state()[0][1] - domain.prize_point()[0][1]
