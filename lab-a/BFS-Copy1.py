...
# def maze():
#     columns= Domain(maze).count_columns()
#     rows= Domain(maze).count_rows()
...
def BFS(state):
    ...
    while goal_test(curr_node.get_state())!=True:
        count+=1
        if goal_test(curr_node.get_state())==True:
            return "WE MADE IT"
        curr_node = Q.dequeue()
        if curr_node.get_state() in parents:
            continue
        ...
        while i <len(list_of_children):
            ...
            if new_node_1.get_state() not in parents:#parents is not working properly
                Q.enqueue(new_node_1)
            i+=2
