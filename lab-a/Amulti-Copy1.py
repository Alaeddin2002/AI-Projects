...
class PriorityQueue:
    ...
    ### push item and priority to the queue
...
def starting_state(maze):#output from function of domain create maze
    ...
    starting_state = State((starting[0],starting[1]),cheese)#changed from [cheese]
...
def successor_function(state):#only valid set of instructions
    ...
    if (Position_check[0],Position_check[1]-1) in empty_spaces:#west
        cheese = copy.deepcopy(state[1])
        if (Position_check[0],Position_check[1]-1) in cheese:
            cheese.remove((Position_check[0],Position_check[1]-1))
    ...
    if (Position_check[0]+1,Position_check[1]) in empty_spaces:#south
        cheese = copy.deepcopy(state[1])
        if (Position_check[0]+1,Position_check[1]) in cheese:
            cheese.remove((Position_check[0]+1,Position_check[1]))
    ...
    if (Position_check[0],Position_check[1]+1) in empty_spaces:
        cheese = copy.deepcopy(state[1])
        if (Position_check[0],Position_check[1]+1) in cheese:
            cheese.remove((Position_check[0],Position_check[1]+1))#nned to change this to remove the cheese founded
...
class Node:
    __slots__ = '_state','_move','_parent','_heuristic','_cost'
    def __init__(self, state=None, move=None, parent=None,heuristic=None, cost =0):
            self._state = state
...
Pq = PriorityQueue()
# hd["two"] = 2# item inside and priotity outside

# node
def heuristic(node):

    price = Domain(maze).prize_point()
    costs=[]
    totals = []
    everything = []
    min_list = []
    ...
    for i in range(len(price)):
        heuristic = node.get_state()[0][0] - price[i][0]
        heuristic2 = node.get_state()[0][1] - price[i][1]
        cost =  (abs(heuristic) + abs(heuristic2))
        costs.append(cost)

    for j in range(len(price)):
        for i in price:
            if i != price[j]:
                total1 = i[0] - price[j][0]
                total2 = i[1] - price[j][1]
                cost2 =  (abs(total1) + abs(total2))
                totals.append(cost2)

    for i in range(len(price)-1):
        everything.append(totals[i*len(price):len(price)*(i+1)])

    for i in range(len(everything)):
        min_list.append(min(everything[i]))

    for i in range(len(min_list)):
        if min_list[i] > costs [i]:
            min_list[i] = costs[i]

    node.set_heuristic(sum(min_list))
...
def A_star(state):
    ...
    visited = []
    heuristic(curr_node)
    cost = curr_node.get_heuristic() + curr_node.get_cost()
    count = 0
    Pq.push(curr_node,cost)


    while goal_test(curr_node.get_state())!=True:
        count += 1
        curr_node = Pq.pop()
        if curr_node.get_state() in visited:
            continue
        #print("get state",curr_node.get_state())
        visited.append(curr_node.get_state())
        ...
        while i <len(list_of_children):
            ...
            heuristic(new_node_1)
            new_node_1.set_cost(curr_node.get_cost()+1)
            cost = new_node_1.get_heuristic()+new_node_1.get_cost()
            if new_node_1.get_state() not in visited:
                Pq.push(new_node_1,cost)

                # Pq[new_node_1]=new_node_1.get_cost()
            i+=2
        # print(count) nodes expanded
    if curr_node.get_state() != curr_node._parent.get_state():
        return curr_node
