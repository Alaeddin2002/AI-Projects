...
class Node:
    __slots__ = '_state','_move','_parent','_cost'
    def __init__(self, state=None, move=None, parent=None,cost=None):
            self._state = state
...
def GBS(state):
    ...
    visited = []
    heuristic(curr_node)
    Pq.push(curr_node,curr_node.get_cost())
    count = 0

    while curr_node.get_cost()>0:
        count += 1
        ...
        while i <len(list_of_children):
            ...
            if new_node_1.get_state() not in visited:
                Pq.push(new_node_1,new_node_1.get_cost())
            i+=2
